...
export default function TransactionsScreen() {
  ...
  useEffect(() => {
    if (activeFilter === 'all') {
      ...
      const filtered = transactions.filter(transaction => transaction.type === activeFilter);
  ...
  const handleActionSubmit = async () => {
    ...
    try {
      ...
      let url = '';
      let data: any = { amount: Number(amount) };
      ...
      Alert.alert(
        "Success",
        `${currentAction?.charAt(0).toUpperCase()}${currentAction?.slice(1)} of $${amount} completed successfully!`
      );

      await fetchTransactions();

    } catch (error: any) {
      const errorMessage = error?.response?.data?.message || error.message || `Failed to ${currentAction}`;
      Alert.alert("Error", errorMessage);
      setActionLoading(false);
    }
  };
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <View style={[styles.modalIconContainer, { backgroundColor: config.color }]}>
  ...
  const renderTransaction = ({ item }: { item: Transaction }) => {
    ...
    return (
      <View style={[styles.transactionItem, { borderLeftColor: details.borderColor }]}>
        <View style={styles.transactionLeft}>
          <View style={[styles.transactionIconContainer, { backgroundColor: details.color }]}>
