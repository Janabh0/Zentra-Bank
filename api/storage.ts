...
const storeToken = async (token: string) => {
  ...
  console.log("🔐 Storage: Token to store:", token ? `${token.substring(0, 10)}...` : "null/undefined");
  ...
  try {
    ...
    console.log("🔍 Storage: SecureStore verification:", verification ? "✅ Found" : "❌ Not found");
    
    if (verification) {
      return; // Success with SecureStore
    }
  } catch (secureError) {
    console.log("⚠️ Storage: SecureStore failed, trying AsyncStorage fallback...");
  ...
  try {
    ...
    console.log("🔍 Storage: AsyncStorage verification:", verification ? "✅ Found" : "❌ Not found");
    
  } catch (asyncError) {
    console.error("❌ Storage: Both SecureStore and AsyncStorage failed!");
    console.error("AsyncStorage error:", asyncError);
    throw new Error("Failed to store token in both SecureStore and AsyncStorage");
  }
};
...
const getToken = async () => {
  ...
  try {
    ...
    if (secureToken) {
      console.log("🔐 Storage: Retrieved from SecureStore:", `${secureToken.substring(0, 10)}...`);
      console.log("🔐 Storage: Token length:", secureToken.length);
      return secureToken;
    }
    console.log("🔐 Storage: No token in SecureStore, trying AsyncStorage...");
  } catch (secureError) {
    console.log("⚠️ Storage: SecureStore retrieval failed, trying AsyncStorage...");
    console.log("SecureStore error:", secureError);
  }
  ...
  try {
    ...
    if (asyncToken) {
      console.log("🔐 Storage: Retrieved from AsyncStorage:", `${asyncToken.substring(0, 10)}...`);
